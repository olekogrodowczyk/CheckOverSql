...
export class SolvingClient {
   ...
   * @return Success
   */
  checkSolving(body: CheckSolvingCommand | undefined): Observable<Int32Result> {
    let url_ = this.baseUrl + '/api/Solving/CheckSolving';
    url_ = url_.replace(/[?&]$/, '');
    ...
    return this.http
      ...
      .pipe(
        _observableMergeMap((response_: any) => {
          return this.processCheckSolving(response_);
        })
      )
      .pipe(
        _observableCatch((response_: any) => {
          if (response_ instanceof HttpResponseBase) {
            try {
              return this.processCheckSolving(response_ as any);
            } catch (e) {
              return _observableThrow(e) as any as Observable<Int32Result>;
            }
          } else
...
export class CheckSolvingCommand implements ICheckSolvingCommand {
  solvingId?: number;
  remarks?: string | undefined;
  points?: number;

  constructor(data?: ICheckSolvingCommand) {
    if (data) {
      for (var property in data) {
        if (data.hasOwnProperty(property))
          (<any>this)[property] = (<any>data)[property];
  ...
  init(_data?: any) {
    if (_data) {
      this.solvingId = _data['SolvingId'];
      this.remarks = _data['Remarks'];
      this.points = _data['Points'];
    }
  }

  static fromJS(data: any): CheckSolvingCommand {
    data = typeof data === 'object' ? data : {};
    let result = new CheckSolvingCommand();
    result.init(data);
    return result;
  }
...
export interface ICheckSolvingCommand {
  solvingId?: number;
  remarks?: string | undefined;
  points?: number;
}
